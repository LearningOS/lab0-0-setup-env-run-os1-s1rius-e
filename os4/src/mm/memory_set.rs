...
impl MemorySet {
    ...
    pub fn delete_frame_area(
        ...
        let area_index = self.find(start_va, end_va);
        match area_index {
            Some(index) => {
                self.remove(index);
    ...
    fn find(&mut self, start_va: VirtAddr, end_va: VirtAddr) -> Option<usize> {
        for i in 0..self.areas.len() {
            if self.areas[i].is_same(start_va, end_va) {
                return Some(i);
            }
        }
        None
    ...
    fn remove(&mut self, area_index: usize) {
        self.areas[area_index].unmap(&mut self.page_table);
    }
